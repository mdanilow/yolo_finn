...
from models.finn_models import *
...
def get_model(cfg_path, weights_path=None, num_classes=80, device='cpu', backbone_only=False, in_channels=3, load_ema=False, bn_from_ema=False):
    ...
    return model.to(device), ckpt, arch_changed
...
class Model(nn.Module):
    def __init__(self, cfg='yolor-csp-c.yaml', ch=3, nc=None, anchors=None, backbone_only=False):  # model, input channels, number of classes
        ...
        self.features_to_save = self.yaml.get('features_to_save') or []
        self.saved_features = []
        ...
        self.model, self.save = parse_model(deepcopy(self.yaml), ch=[ch], backbone_only=backbone_only)  # model, savelist
...
def parse_model(d, ch, backbone_only=False):  # model_dict, input_channels(3)
    ...
    if backbone_only:
        ...
        to_parse = d['backbone'] + d['head']

    for i, (f, n, m, args) in enumerate(to_parse):  # from, number, module, args
        ...
        if m in [nn.Conv2d, ConvBNReLU, Conv, RobustConv, RobustConv2, DWConv, GhostConv, RepConv, RepConv_OREPA, DownC, 
                 ...
                 InvertedResidual, QuantConvBNReLU, QuantInvertedResidual, QuantDwsConv, DwsConv,
                 C2f, QuantConv, QuantC2f, QuantBottleneck, QuantSPPF]:
            c1, c2 = ch[f], args[0]
            ...
            c2 = ch[f] // 2
        elif m in [Detect, QuantDetect, IDetect, IAuxDetect, IKeypoint]:
            args.append([ch[x] for x in f])
            if isinstance(args[1], int):  # number of anchors
                args[1] = [list(range(args[1] * 2))] * len(f)
        elif m in [V8Detect, QuantV8Detect]:
            args.append([ch[x] for x in f])
        elif m is ReOrg:
            c2 = ch[f] * 4
        elif m is Contract:
