...
def train(hyp, opt, device, tb_writer=None):
    ...
    if not opt.resume:
        shutil.copy(opt.cfg, join(save_dir, 'cfg.yaml'))
    ...
    imgsz, imgsz_test = opt.img_size[0], opt.img_size[1]
...
if __name__ == '__main__':
    ...
    parser.add_argument('--fresh_optimizer', action='store_true', help='whether to ignore optimizer state from pretrained checkpoint')
    parser.add_argument('--fresh_ema', action='store_true', help='whether to ignore ema state from pretrained checkpoint')
    ...
    if opt.resume and not wandb_run:  # resume an interrupted run
        ...
        apriori = opt.global_rank, opt.local_rank, opt.fresh_optimizer, opt.fresh_ema, opt.init_from_ema
        with open(Path(ckpt).parent.parent / 'opt.yaml') as f:
            opt = argparse.Namespace(**yaml.load(f, Loader=yaml.SafeLoader))  # replace
        config_path = str(Path(ckpt).parent.parent / 'cfg.yaml')
        opt.cfg, opt.weights, opt.resume, opt.batch_size, opt.global_rank, opt.local_rank, opt.fresh_optimizer, opt.fresh_ema, opt.init_from_ema = config_path, ckpt, True, opt.total_batch_size, *apriori  # reinstate
